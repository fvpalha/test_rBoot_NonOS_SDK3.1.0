...
serverConnData tcpconnData[MAX_CONN];


static serverConnData ICACHE_FLASH_ATTR *serverFindConnData(void *arg) {
	int i;
	for (i=0; i<MAX_CONN; i++) {
		if (tcpconnData[i].conn==(struct espconn *)arg) 
			return &tcpconnData[i];
	}
...
static void ICACHE_FLASH_ATTR serverReconCb(void *arg, sint8 err) {
	serverConnData *conn=serverFindConnData(arg);
//	os_printf("server.c - serverRecnCb - on conn %p\n", conn);
	if (conn==NULL) return;
...
static void ICACHE_FLASH_ATTR serverDisconCb(void *arg) {
	serverConnData *conn=serverFindConnData(arg);
//	os_printf("server.c - serverDisconCb - on conn %p\n", conn);

	//Just look at all the sockets and kill the slot if needed.
	int i;
	for (i=0; i<MAX_CONN; i++) {
		if (tcpconnData[i].conn!=NULL) {
//			os_printf("   udpconnData[i].conn->state conn %p\n", udpconnData[i].conn->state);
			if (tcpconnData[i].conn->state==ESPCONN_NONE || tcpconnData[i].conn->state==ESPCONN_CLOSE) {
				tcpconnData[i].conn=NULL;
			}
		}
	}
}
...
static void ICACHE_FLASH_ATTR serverConnectCb(void *arg) {
	...
	for (i=0; i<MAX_CONN; i++) if (tcpconnData[i].conn==NULL) break;
	...
	tcpconnData[i].conn=conn;
	tcpconnData[i].txbufferlen = 0;
	tcpconnData[i].readytosend = true;
...
void ICACHE_FLASH_ATTR serverInit(int port) {
	...
	for (i = 0; i < MAX_CONN; i++) {
		tcpconnData[i].conn = NULL;
		tcpconnData[i].txbuffer = txbuffer[i];
		tcpconnData[i].txbufferlen = 0;
		tcpconnData[i].readytosend = true;
	}
	...
	espconn_tcp_set_max_con_allow(&serverConn, 1); //Allow 1 connection max
}
